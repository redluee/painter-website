<div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 md:p-8">
    ...
    <form id="project-form" class="space-y-6">
        <div>
            ...
            <input type="text" id="name" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-accent-1 focus:border-transparent" placeholder="Bijv. Historisch Grachtenpand">
        </div>
        <div>
        ...
        <div class="flex gap-3">
            <button type="submit" class="px-8 py-3 bg-accent-1 text-white text-sm font-medium rounded-xl hover:bg-accent-2 transition-colors">Project aanmaken</button>
...
<script>
    ...
    starsContainer.addEventListener('mousemove', function (e) {
        ...
        if (!unit) return;
        var pos = parseInt(unit.dataset.star);
    ...
    starsContainer.addEventListener('click', function (e) {
        ...
        var rect = unit.getBoundingClientRect();
        var isLeft = (e.clientX - rect.left) < rect.width / 2;
        selectedStars = isLeft ? pos - 0.5 : pos;
        reviewStarsInput.value = selectedStars;
        updateStars(selectedStars);
    });

    var descriptionEditor = new Quill('#description-editor', { theme: 'snow', modules: { toolbar: [['bold', 'italic'], [{ list: 'bullet' }], ['clean']] } });
    ...
    async function handleFiles(files) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    processingEntries.delete(processId);
                    renderPreviews();
                    successCount++;
                } else {
                ...
                lastError = e instanceof Error ? e.message : 'Onbekende fout';
                processingEntries.delete(processId);
                renderPreviews();
                errorCount++;
            }
        ...
        else if (successCount > 0 || hasProcessing) { hideError(); }
    ...
    function renderPreviews() {
        ...
        uploadedUrls.forEach(function (url, i) {
            ...
            var sizeBadge = size !== undefined ? '<span class="absolute top-2 left-2 z-10 px-1.5 py-0.5 bg-green-600/80 text-white text-[10px] font-medium rounded pointer-events-none">' + ImageUtils.formatFileSize(size) + '</span>' : '';
            html += '<div class="relative group aspect-[4/3] rounded-lg overflow-hidden bg-gray-100 cursor-grab" draggable="true" data-url="' + url + '">' + sizeBadge +
                '<img src="' + url + '" class="w-full h-full object-cover pointer-events-none" alt="">' +
                '<button type="button" data-url="' + url + '" class="absolute top-2 right-2 w-7 h-7 bg-red-600 text-white rounded-full text-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-700 z-10">&times;</button></div>';
        });
        imagePreviews.innerHTML = html;
        imagePreviews.querySelectorAll('button').forEach(function (btn) {
            btn.addEventListener('click', function () {
                uploadedUrls = uploadedUrls.filter(function (u) { return u !== btn.dataset.url; });
                compressedSizes.delete(btn.dataset.url);
                renderPreviews();
            });
        });
    }

    imagePreviews.addEventListener('dragstart', function (e) {
        var item = e.target.closest('[draggable]');
        if (!item) return; draggedUrl = item.dataset.url; e.dataTransfer.effectAllowed = 'move'; item.classList.add('opacity-50');
    });
    imagePreviews.addEventListener('dragover', function (e) {
        e.preventDefault(); e.dataTransfer.dropEffect = 'move';
        var item = e.target.closest('[draggable]');
        if (item && item.dataset.url !== draggedUrl) item.classList.add('ring-2', 'ring-accent-1');
    });
    ...
    imagePreviews.addEventListener('drop', function (e) {
        ...
        var fromIndex = uploadedUrls.indexOf(draggedUrl), toIndex = uploadedUrls.indexOf(target.dataset.url);
        if (fromIndex === -1 || toIndex === -1) return;
        var moved = uploadedUrls.splice(fromIndex, 1)[0]; uploadedUrls.splice(toIndex, 0, moved); renderPreviews(); draggedUrl = null;
    });
    imagePreviews.addEventListener('dragend', function () {
        draggedUrl = null; imagePreviews.querySelectorAll('[draggable]').forEach(function (el) { el.classList.remove('opacity-50', 'ring-2', 'ring-accent-1'); });
    });
    ...
    form.addEventListener('submit', async function (e) {
        ...
        if (!descriptionEditor.getText().trim()) { showError('Het veld "Beschrijving" is verplicht.'); return; }
        ...
        var payload = {
            ...
            description: descriptionEditor.root.innerHTML, pictures: uploadedUrls,
            review: selectedStars > 0 ? { stars: selectedStars, description: reviewDesc } : null,
        };
        try {
            var res = await fetch('/api/admin/projects', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
            if (res.ok) { window.location.href = '/admin/projects'; } else { var data = await res.json(); showError(data.error || 'Opslaan mislukt'); }
